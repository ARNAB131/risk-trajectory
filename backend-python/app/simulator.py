...
def generate_vitals(profile: str, state: Dict) -> Dict[str, float]:
    """
    Generates semi-realistic vitals with drift; allows occasional deteriorations.
    state holds last values for continuity.
    """
    if not state:
        # initial baselines
        if profile == "athlete":
    ...
    # occasional deterioration events
    ...
    # clamp
    state["hr"] = clamp(state["hr"], 40, 190)
    state["sys"] = clamp(state["sys"], 90, 220)
    state["dia"] = clamp(state["dia"], 50, 140)
    state["spo2"] = clamp(state["spo2"], 75, 100)
    state["temp"] = clamp(state["temp"], 34.0, 41.0)
